...
class EmailTest {
    ...
    fun `should create e-mail`() {
        ...
        assertTrue("list of notification should be empty", email.validators().filter { it.isPresent }.map { it }.isEmpty())
    ...
    fun `should validate creation email with empty value`() {
        val email = Email("")
        assertTrue("list of notification should not be empty", email.validators().filter { it.isPresent }.map { it }.isNotEmpty())
        val expectedNotification = Notification(notification = "Email is required.")
        val receivedNotification = email.validators().filter { it.isPresent }.map { it.get() }.map { it }.first()
